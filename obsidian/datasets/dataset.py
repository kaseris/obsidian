...
class DeepFashionCategoryAttribute(Dataset):
    ...
    Args:
        data_dir (str, optional): The directory containing the DeepFashion dataset. Defaults to `obsidian.core.config.DEEP_FASHION_DIR`.
    ...
    Methods:
        __init__(self, data_dir=obsidian.core.config.DEEP_FASHION_DIR, include_attributes=False, transforms=None, split_type='train', split_info=None, garment_annotations=None):
    ...
    def __init__(self,
        ...
        DEEP_FASHION_CLOTHING_ANNOS_DIR = osp.join(
            self.data_dir, 'Anno_coarse')
        DEEP_FASHION_CLOTHING_IMAGES_DIR = osp.join(self.data_dir, 'img')
        DEEP_FASHION_CLOTHING_CATEGORIES_PATH = osp.join(
            DEEP_FASHION_CLOTHING_ANNOS_DIR, 'list_category_cloth.txt')
        DEEP_FASHION_CLOTHING_ATTRIBUTES_PATH = osp.join(
            DEEP_FASHION_CLOTHING_ANNOS_DIR, 'list_attr_cloth.txt')
        DEEP_FASHION_CLOTHING_LIST_CAT_IMG_PATH = osp.join(
            DEEP_FASHION_CLOTHING_ANNOS_DIR, 'list_category_img.txt')
        DEEP_FASHION_CLOTHING_LIST_ATT_IMG_PATH = osp.join(
            DEEP_FASHION_CLOTHING_ANNOS_DIR, 'list_attr_img.txt')
        DEEP_FASHION_CLOTHING_LIST_BBOX_IMG_PATH = osp.join(
            DEEP_FASHION_CLOTHING_ANNOS_DIR, 'list_bbox.txt')
        DEEP_FASHION_EVAL_DIR = osp.join(self.data_dir, 'Eval')
        DEEP_FASHION_SPLITS_PATH = osp.join(
            DEEP_FASHION_EVAL_DIR, 'list_eval_partition.txt')
        ...
        self.IDX_TO_CLASS = {idx + 1: c for idx,
                             c in enumerate(self.CLASS_LABELS)}
    ...
    def __getitem__(self, index) -> Dict[torch.TensorType,
        ...
        try:
            ...
            img = Image.open(osp.join(self.data_dir, pth.replace(
                'A-Line_Dress', 'A-line_Dress'))).convert('RGB')
...
class DeepFashion2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = [anno[item].get('category_id')
        ...
        for item in self._get_garment_annos(anno):
            ...
            for polygon in segmentation_mask:
                ...
                ImageDraw.Draw(mask).polygon(
                    tuple(map(tuple, polygon)), outline=cat_id, fill=cat_id)
                mask_pil = mask
        return mask_pil
