...
class RoIPooling(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_dim = spatial_dim # width and height of the output of the previous layer
        self.roi_pool_size = roi_pool_size # desired output width & height
    
        self.reduce_1x1 = nn.Conv2d(self.feats_dim, self.n_channels, kernel_size=1, stride=1)
    ...
    def forward(self, features: torch.Tensor):
        ...
        batch_indices = torch.arange(0, batch_size).view(-1, batch_size).t().float()
    ...
    def __init__(self,
        ...
        for ch in gd_config:
            if ch.upper() == 'S':
                ...
                self.global_descriptors.append(GlobalDescriptor(p=float('inf')))
                self.fc_layers.append(nn.Linear(fan_in, k, bias=False))
            else:
